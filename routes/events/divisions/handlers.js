...
module.exports.getDivision = (req, res) => {
    ...
    if(division !== null) {
      ...
      res.status(404);
      res.json({
        msg: "There is no division with the id of " + req.params.id,
      });
    }
  }).catch((error) => {
    res.status(500);
    res.json({
      msg: "Error finding division " + req.params.id + ": " + error,
    });
  });
}
...
module.exports.updateDivision = (req, res) => {
	db.EventDivision.update(
      ...
      event_id: req.body.event_id,
		...
		res.status(500);
    res.json({
      msg: "Error updating division " + req.params.id + ": " + error
    });
	});
}
...
module.exports.deleteDivision = (req, res) => {
    ...
    res.json({
      msg: "Division " + req.params.id + " deleted."
    });
  }).catch((error) => {
    res.status(500);
    res.json({
      msg: "Error deleting division " + req.params.id + ": " + error
    });
  });
}
